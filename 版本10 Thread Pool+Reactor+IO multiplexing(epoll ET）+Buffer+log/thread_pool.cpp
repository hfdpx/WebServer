#include "thread_pool.h"
#include "mutex.h"

#include "rlog.h"
//线程池构造函数
ThreadPool::ThreadPool(int threadNum, int maxQueueSize)
    : threadNum_(threadNum)
    , maxQueueSize_(maxQueueSize)
    , mutex_()
    , notEmpty_(mutex_)
    , notFull_(mutex_)

{
    //保证程序正确运行
    assert(threadNum >= 1 && maxQueueSize >= 1);

    DEBUG("%d threads in the thread pool will be created",threadNum);//log

    /* 接下来要构建threadNum个线程 */
    pthread_t tid_t;
    for (int i = 0; i < threadNum; i++)
    {
        Pthread_create(&tid_t, NULL, startThread, this);
    }
    DEBUG("%d threads in the thread pool have been created",threadNum);//log
}

size_t ThreadPool::queueSize()
{
    MutexLockGuard lock(mutex_);//获得任务队列锁
    return queue_.size();
}

ThreadPool::~ThreadPool()
{

}

//线程运行时函数
void* ThreadPool::startThread(void* obj)
{
    DEBUG("The current thread pool thread [%lu] is running",pthread_self());

    //子线程和主线程分离 以便子线程的资源回收
    Pthread_detach(Pthread_self());

    //显式类型转换
    ThreadPool* pool = static_cast<ThreadPool*>(obj);

    //回调运行线程池运行函数
    pool->run();
    return pool;
}

//从任务队列中获得任务
ThreadPool::Task ThreadPool::take()
{
    MutexLockGuard lock(mutex_);//先获得任务队列的锁

    //若任务队列为空，则阻塞等待
    while (queue_.empty())
    {
        notEmpty_.wait();
    }
    Task task;

    //获得任务
    if (!queue_.empty())
    {
        //获得队列首部的任务
        task = queue_.front();
        queue_.pop_front();

        //通知线程池任务队列不满,以便添加任务
        if (maxQueueSize_ > 0)
        {
            notFull_.notify();
        }
    }
    //mylog("threadpool take 1 task!");
    DEBUG("One task taken away");
    return task;
}

//线程池运行函数
void ThreadPool::run()
{
    for ( ; ; )   /* 一直运行下去 */
    {
        //获得任务以及获得执行该任务的执行函数
        Task task(take());

        //该任务的执行函数存在
        if (task)
        {
            //mylog("task run!");
            DEBUG("One task will run");
            task();
            DEBUG("The end of task run");
        }
        //mylog("task over!");
    }
}

//判断任务队列是否满
bool ThreadPool::isFull()
{
    mutex_.assertLocked();//锁不为本线程持有则退出当前线程
    return maxQueueSize_ > 0 && queue_.size() >= maxQueueSize_;
}


//添加任务 到 任务队列
bool ThreadPool::append(Task&& task)
{
    /* 使用了右值引用 */
    {
        MutexLockGuard lock(mutex_); /* 首先加锁 */
        while (isFull())
        {
            notFull_.wait(); /* 等待queue有空闲位置 */
            INFO("The task queue is full and waiting for an idle location");
        }

        assert(!isFull());

        INFO("The task queue have free location");

        //任务加入任务队列
        queue_.push_back(std::move(task)); /* 直接用move语义,提高了效率 */

        //mylog("put task onto queue!");
        INFO("A task is placed in the task queue");

    }
    notEmpty_.notify(); /* 通知任务队列中有任务可做了 */
}
