...
/*
   本质就是accept函数,加入了些许错误处理，变成了包裹函数，比较方便而已
...
1.调用socket函数:  建立一个套接字,该套接字接下来用于网络通信

2.调用bind函数:    将该套接字绑定到一个地址端口
...
int open_listenfd(int port)
{
	...
	struct sockaddr_in serveraddr;//地址信息

	//构建一个socket
	//AF_INET采用IPV4 SOCK_STREAM采用TCP
	if ((listenfd = socket(AF_INET, SOCK_STREAM, 0)) < 0)
		return -1;

    /*设置套接字:closesocket后想继续重用该socket
      listenfd:   套接字名称
      SQL_SOCKET:通用套接字选项 指定控制套接字的层次
      SO_REUSEADDR:允许重用本地址和端口
    */
    //设置端口复用
	...
	bzero((char *)&serveraddr, sizeof(serveraddr));//初始化成0

	//地址信息参数设置
	serveraddr.sin_family = AF_INET;//采用IPV4
	serveraddr.sin_addr.s_addr = htonl(INADDR_ANY);//表示服务器上的多个本地IP地址都进行绑定端口号(因为多网卡就有多个IP地址)
	serveraddr.sin_port = htons((unsigned short)port);//设置端口号
