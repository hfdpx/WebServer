...
{
    DEBUG("An HTTP request will be parse(Parse)");
    bool ok = true;
    bool hasMore = true;
    while (hasMore)
    {
        if (state_ == RequestParseState::kExpectRequestLine)
        {
            ...
            else
            {
                ...
                WARN("An HTTP parsing error(Parse)");
        ...
        {
            WARN("A connection needs to parse the body ,but this function is not implemented");
        }
    }
    DEBUG("An HTTP request have been parsed");
    return state_;
}
