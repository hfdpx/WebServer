...
struct utc_timer
{
    ...
    uint64_t get_curr_time(int* p_msec = NULL)
    {
        ...
        {
            sec = tv.tv_sec % 60;
            _sys_acc_sec = tv.tv_sec;   //replace
...
private:
    ...
    uint64_t _sys_acc_min;//the number of seconds the system  passes when writing a log
    uint64_t _sys_acc_sec;//the number of minutes the system  passes when writing a log
};
...
public:
    ...
    cell_buffer(uint32_t len):   //class assigns values to member variables
    status(FREE),
    prev(NULL),
    next(NULL),
    _total_len(len),
    ...
    uint32_t avail_len() const { return _total_len - _used_len; }  //usable length

    bool empty() const { return _used_len == 0; }

    void append(const char* log_line, uint32_t len)  //add date
    {
        if (avail_len() < len)
            return ;
...
private:
    ...
    cell_buffer& operator=(const cell_buffer&); //reload =
...
public:
    ...
    static ring_log* ins()
    {
        pthread_once(&_once, ring_log::init); //execute once ,ensure thread-safe
        return _ins;
    }
...
private:
    ...
    const ring_log& operator=(const ring_log&); //reload =
...
void* be_thdo(void* args); //thread runtime function

//set ring_log of one buffer length
