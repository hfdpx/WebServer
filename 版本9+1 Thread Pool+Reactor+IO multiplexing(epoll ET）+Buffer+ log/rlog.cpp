...
pthread_mutex_t ring_log::_mutex = PTHREAD_MUTEX_INITIALIZER; //initiate static
...
ring_log::ring_log():
    _buff_cnt(3),
    _curr_buf(NULL),   //producer buffer
    _prst_buf(NULL),   //customer buffer
    _fp(NULL),
    _log_cnt(0),
    _env_ok(false),
    _level(INFO),
...
void ring_log::init_path(const char* log_dir, const char* prog_name, int level)
{
    ...
    mkdir(_log_dir, 0777);
    //check to see if this directory exists ,and if file creation is allowed under it
...
void ring_log::persist()
{
    while (true)
    {
        ...
        {
            struct timespec tsp;  //different precision
            struct timeval now;
        ...
        fflush(_fp);//refresh buff,date in the buffer is written to the file immediately,without waiting for program to finish
...
void ring_log::try_append(const char* lvl, const char* format, ...)
{
    ...
    else
    {
        ...
        else//_curr_buf->status == cell_buffer::FULL, assert persist is on here too!      //discard a part log ,should be improve
        {
            _lst_lts = curr_sec;
        }
    }
...
bool ring_log::decis_file(int year, int mon, int day)
{
    ...
    else if (ftell(_fp) >= LOG_USE_LIMIT)
    {
        ...
        for (int i = _log_cnt - 1;i > 0; --i)
        {
            ...
            rename(old_path, new_path);  //file move
        }
